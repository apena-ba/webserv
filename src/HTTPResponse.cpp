...
template<typename T>
static std::string tostr(T thang) {
    std::string ret;
...
void HTTPResponse::get_perform(const Configuration &conf) {
    ...
    this->_body.assign(std::istreambuf_iterator<char>(file),
                       std::istreambuf_iterator<char>()); // Put the whole file into the body string.
    file.close();
...
HTTPResponse::HTTPResponse(const HTTPRequestParser &givenRequest, const Configuration &conf) : HTTPRequestParser(
        givenRequest) {
    ...
    // 2:
    this->_response =
            this->_vals["version"] + " " + this->_strStatus + " " + this->_statusMessages[this->_strStatus] + "\r\n";
