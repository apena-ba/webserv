...
/*   Updated: 2023/05/02 18:39:43 by apena-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool HTTPRequestParser::parsefirstline(const std::string &req, uint32_t &i) {
    ...
    if (!req.compare(i, 7, "http://")) {
        ...
        std::cerr << ">> Error: 400, bad absolute location." << std::endl;
        this->_status = 400;
        return false;
    } else
        host = "";
    ...
    if (version.compare(5, 4, "1.0") && version.compare(5, 4, "1.1") && version.compare(5, 2, "2") &&
        version.compare(5, 2, "3")) {
